...
public class MessagesServiceImpl extends ServiceImpl<MessagesMapper, MessagesDo> implements IMessagesService {
    ...
    public List<MessagesDo> queryPendingMessages(String topicName, long time) {
        LambdaQueryWrapper<MessagesDo> wrapper = new LambdaQueryWrapper<MessagesDo>()
                .eq(MessagesDo::getStatus, MessagesStatusEnum.PENDING.name())
                .eq(MessagesDo::getTopic, topicName)
                .lt(MessagesDo::getEnableConsumeTime, time);
